...
class ElnImportWidget(ipw.VBox):
    ...
    def __init__(self, path_to_root="../", **kwargs):
        ...
        if eln is None:
            url = f"{path_to_root}aiidalab-eln/eln_configure.ipynb"
...
class ElnExportWidget(ipw.VBox):
    ...
    def __init__(self, path_to_root="../", **kwargs):
        ...
        if self.eln:
            ...
            self.message.value = f"""Warning! The access to ELN is not configured. Please follow <a href="{self.path_to_root}/aiidalab-eln/eln_configure.ipynb" target="_blank">the link</a> to configure it.</br> </br> More details: {msg}"""
    ...
    def send_to_eln(self, _=None):
        if self.eln and self.eln.is_connected:
            ...
            self.message.value = f"""\u274c Something isn't right! We were not able to send the data to the "<strong>{self.eln.eln_instance}</strong>" ELN instance. Please follow <a href="{self.path_to_root}/aiidalab-eln/eln_configure.ipynb" target="_blank">the link</a> to update the ELN's configuration."""
    ...
    def handle_output(self, _=None):
        with self._output:
            ...
            if self.modify_settings.value:
                display(
                    ipw.HTML(
                        f"""Currently used ELN is: "<strong>{self.eln.eln_instance}</strong>". To change it, please follow <a href="{self.path_to_root}/aiidalab-eln/eln_configure.ipynb" target="_blank">the link</a>."""
                    )
                )
