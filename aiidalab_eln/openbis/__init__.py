...
class OpenbisElnConnector(ElnConnector):
    ...
    def create_new_collection_openbis(self, project_code, collection_code, collection_type, collection_name):
        collection = self.session.new_collection(project = project_code, code = collection_code, type = collection_type)
    ...
    def check_if_collection_exists(self, project_code, collection_code):
        return self.session.get_collections(project = project_code, code = collection_code).df.empty == False
    
    def create_collection_openbis(self, project_code, collection_name, collection_code, collection_type, collection_exists):
        if collection_exists == False:
            collection = self.create_new_collection_openbis(project_code, collection_code, collection_type, collection_name)
        else:
        ...
        for _, aiida_object in enumerate(aiida_objects):
            ...
            for openbis_object in openbis_objects:
                if openbis_object.props.get("wfms-uuid") == aiida_object.uuid:
                    ...
                    print(f"Object {openbis_object.props.get('wfms-uuid')} already exists.")
    ...
    def create_new_object_openbis(self, collection_identifier, object_type, parents = None):
        if parents == None:
            return self.session.new_sample(collection = collection_identifier, type = object_type)
        else:
            return self.session.new_sample(collection = collection_identifier, type = object_type, parents = parents)
    
    def set_object_props(self, aiida_object_index, aiida_object, number_aiida_objects, openbis_object_type):
        ...
        if openbis_object_type == "ATOMISTIC_MODEL":
            ...
            if number_aiida_objects > 1 and aiida_object_index == number_aiida_objects - 1: # If it is the last of more than one structures, it is optimised.
    ...
    def create_atomistic_models(self, structures_nodes, structures_inside_openbis, collection_identifier):
        atomistic_models = []
        ...
        for structure_index, structure in enumerate(structures_nodes):
            ...
            if "eln" in structure.base.extras.all:
                selected_molecule = self.session.get_sample(structure.base.extras.all["eln"]["molecule_uuid"])
            else:
                ...
                if structure_inside_openbis[1] == False:
                    ...
                    atomistic_model = self.create_new_object_openbis(collection_identifier, "ATOMISTIC_MODEL")
                    atomistic_model.props = self.set_object_props(structure_index, structure, number_aiida_objects, "ATOMISTIC_MODEL")
    ...
    def create_geoopts_simulations(self, geoopts_nodes, geoopts_inside_openbis, collection_identifier, atomistic_models):
        geoopts_simulations = []
        ...
        for geoopt_index, geoopt in enumerate(geoopts_nodes):
            ...
            if geoopt_inside_openbis[1] == False:
                ...
                geoopt_simulation = self.create_new_object_openbis(collection_identifier, "SIMULATION", [parent_atomistic_model])
                ...
                geoopt_model = self.create_new_object_openbis(collection_identifier, "GEO_OPTIMISATION", [geoopt_simulation])
                geoopt_model.save()
    ...
    def create_stm_simulations(self, stms_nodes, stms_inside_openbis, collection_identifier, atomistic_models):
        stms_simulations = []
        ...
        for stm_index, stm in enumerate(stms_nodes):
            ...
            if stms_inside_openbis[0][1] == False:
                ...
                # Simulation object
                stm_simulation = self.create_new_object_openbis(collection_identifier, "SIMULATION", [optimised_atomistic_model])
                ...
                # Simulated STM
                stm_model = self.create_new_object_openbis(collection_identifier, "STM", [stm_simulation])
                ...
                os.system(f"verdi archive create {stm_simulation_dataset_filename} --no-call-calc-backward --no-call-work-backward --no-create-backward -N {stm.pk}")
    ...
    def export_data(self):
        ...
        atomistic_models_collection_identifier = f"{inventory_project_code}/{atomistic_models_collection_code}"
        ...
        _ = self.create_collection_openbis(inventory_project_code, atomistic_models_collection_name, atomistic_models_collection_code, atomistic_models_collection_type, atomistic_models_collection_exists)
        ...
        all_structures, all_aiida_geoopts = self.get_all_structures_and_geoopts(self.node)
        
        # Verify which GeoOpts are already in openBIS
        all_aiida_geoopts, all_geoopts_inside_openbis = self.check_aiida_objects_in_openbis(all_aiida_geoopts, simulations_openbis)
        
        # Verify which structures (atomistic models) are already in openBIS
        all_structures, all_structures_inside_openbis = self.check_aiida_objects_in_openbis(all_structures, atomistic_models_openbis)
        ...
        all_atomistic_models = self.create_atomistic_models(all_structures, all_structures_inside_openbis, atomistic_models_collection_identifier)
        
        # Build GeoOpts in openBIS
        all_geoopts_simulations, all_atomistic_models = self.create_geoopts_simulations(all_aiida_geoopts, all_geoopts_inside_openbis, simulation_experiment_identifier, all_atomistic_models)
        ...
        if isinstance(self.node, orm.WorkChainNode):
            ...
            all_stms_simulations = self.create_stm_simulations(all_aiida_stms, all_stms_inside_openbis, simulation_experiment_identifier, all_atomistic_models)
